...
export async function POST(request: Request) {
  ...
  const subject = `New Contact Message — ${name}${service ? ` (${service})` : ''}`
  const html = `
  ...
  if (!isEmailConfigured()) {
    ...
    // log it so it's recoverable from server logs until delivery is configured.
    console.warn('[contact] Email not configured — lead not delivered:', { name, phone, email, service, message })
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: 'Could not send your message. Please call us directly.' },
      { status: 502 },
    )
  }
