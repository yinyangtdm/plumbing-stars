...
- **Wrapper re-exports** like [components/HeaderWrapper.tsx](components/HeaderWrapper.tsx) (`export { default } from './X'`) exist to cross the RSC→client boundary cleanly. Import the wrapper from server components.
- **Leaflet map** is a three-file chain — edit the right layer: [ServiceAreaMapClientWrapper.tsx](components/ServiceAreaMapClientWrapper.tsx) just re-exports (server-importable) → [ServiceAreaMapClient.tsx](components/ServiceAreaMapClient.tsx) is `'use client'` and does the `dynamic(() => import('./ServiceAreaMap'), { ssr: false })` (Leaflet can't SSR) → [ServiceAreaMap.tsx](components/ServiceAreaMap.tsx) is the actual map: it dynamically `import('leaflet')` inside `useEffect` and manually clears stale `_leaflet_id` to survive React StrictMode double-mounting. Leaflet CSS is loaded via a `<link>` tag, not an import. Its geometry comes from [lib/serviceArea.ts](lib/serviceArea.ts) (the approximate coverage polygon — "north of the 10, west of the 110") and [lib/countyBorders.ts](lib/countyBorders.ts) (real Census county outlines). `countyBorders.ts` still exports `VENTURA_COUNTY_GEO` — that's a map *context* border, not LA-only drift.
